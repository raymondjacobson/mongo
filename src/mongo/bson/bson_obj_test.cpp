...
namespace {
    ...
    TEST(BSONObjCompare, NumberDecimalInfinity) {
        ...
        ASSERT_GT(BSON("" << Decimal128::getPosInfinity()),
                  BSON("" << Decimal128::getNegInfinity()));
        ...
        ASSERT_LT(BSON("" << Decimal128::getNegInfinity()),
                  BSON("" << Decimal128::getPosInfinity()));
    }
    ...
    TEST(BSONObjCompare, NumberDecimalCompareDoubleNoDecimalRepresentation) {
        ...
        ASSERT_EQ(BSON("" << Decimal128("179769313486232E294")), BSON("" << std::numeric_limits<double>::max()));
        ASSERT_EQ(BSON("" << Decimal128("-179769313486232E294")), BSON("" << -std::numeric_limits<double>::max()));

        ASSERT_GT(BSON("" << Decimal128("179769313486233E294")), BSON("" << std::numeric_limits<double>::max()));
        ASSERT_LT(BSON("" << Decimal128("-179769313486231E294")), BSON("" << -std::numeric_limits<double>::min()));
    }
