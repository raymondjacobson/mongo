...
const JSFunctionSpec NumberDecimalInfo::methods[4] = {
...
void NumberDecimalInfo::Functions::valueOf(JSContext* cx, JS::CallArgs args) {
	Decimal128 out = NumberDecimalInfo::ToNumberDecimal(cx, args.thisv());

	args.rval().setDecimal(out);
}
...
void NumberDecimalInfo::construct(JSContext* cx, JS::CallArgs args) {
	...
	if (args.length() == 0) {
		...
		x = ValueWriter(cx, args.get(0)).toString();
	} else {
